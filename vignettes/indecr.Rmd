---
title: "Introducción a indecr"
output: rmarkdown::html_vignette
vignette: >
  %\VignetteIndexEntry{Introducción a indecr}
...
directamente desde R, sin descargar archivos ni conocer los endpoints
de cada API. Todas las funciones devuelven tibbles listos para analizar
con tidyverse.
...
`ipc_chain()` usa el archivo oficial del INDEC que ya incorpora el
empalme entre la base diciembre 2016 y la base diciembre 2022. Es la
función recomendada para análisis históricos largos.
```{r chain}
...
`deflate()` convierte cualquier vector de valores nominales a valores
reales usando el IPC como deflactor. El parámetro `base` define en qué
pesos se expresan los valores resultantes.
```{r deflate}
